...
**Toolkit Hub** is a desktop application that installs, launches and updates
every tool below from one window, so getting the toolkit no longer means
visiting ten GitHub pages and unpacking ten differently-named archives.
...
- **Know when something is out of date.** One request fetches a signed catalog;
  the hub compares it against what you have installed.
- **Every download is checked.** None of these tools is code-signed, so the hub
  pins a SHA-256 for each artifact inside a minisign-signed catalog. A release
  file swapped after the catalog was built fails to install rather than
  installing quietly.
- **Nothing of yours is moved.** The hub manages program files only. Your saves,
  ...
  behaves the same started from the hub or started by hand.
- **Work offline** is a single switch that stops every outbound request,
  including the check on launch. The hub says what it would contact before it
  contacts anything.

It is a library and process manager, not a browser: several of these tools are
hardened loopback web apps that refuse to be framed, and two are Tkinter, so each
one runs as its own process in its own window.
...
| --- | --- |
| Source | [`hub/`](hub) |
| Design | [`docs/hub/design.md`](docs/hub/design.md) |
| Catalog format | [`docs/hub/catalog-schema.md`](docs/hub/catalog-schema.md) |
| Why submodules, not a monorepo | [`docs/adr/0001-repo-topology.md`](docs/adr/0001-repo-topology.md) |

The catalog the hub reads is generated by
[`tools/build_catalog.py`](tools/build_catalog.py) from the per-tool rules in
[`catalog/sources.toml`](catalog/sources.toml), and regenerated by CI whenever a
