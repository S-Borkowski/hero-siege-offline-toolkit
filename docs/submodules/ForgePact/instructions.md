...
| `py -m unittest discover -s tests -v` | `ForgePact/` | PowerShell / CMD | Python 3.10+ | Executes all 178 Python contract tests. | Read-only test execution; all tests pass | Verified |
...
- Example commands:
  ...
  - `reveal 1` / `reveal 0`, `reveal packs 1` / `reveal packs 0` (+ `reveal stat` in the research build only): Map reveal. `reveal` clears the zone's minimap fog grid once per zone identity - measured 2026-09-11 to be all that static icons need (waypoints, dungeon entrances, chests, shrines and mining nodes are fog-gated, **not** gated by `isDiscovered`, so no per-object sweep exists or is needed). `reveal packs` is the monster half and the only part that adds work: most packs do not exist until the player walks within ~1050 px of their `Enemy_Creator_*`, so it opens a bounded 900-frame window per zone during which `distance_to_object` answers 0 for creators, letting the game's own creator logic populate the map (measured 148 → 866 and 208 → 1273 enemies; ~7.8 ms frame average, plugin 2.3%). The window only opens once a live creator reports a real `enemyCreatorTimer` - firing during zone load leaves spawners permanently inert. It is also **bound to the room it was opened for**: the room is re-checked every frame while the window is open (not on the 20-frame identity throttle), and losing the minimap, changing zone identity or changing room shuts it immediately, so a window can never carry over into the next zone's loading creators. Turning `reveal packs` on arms the **current** zone's readiness-gated pass rather than waiting for the next zone change. Panel: `map_reveal` with the nested `map_reveal_packs` checkbox. See `ForgePact/docs/map-reveal-research.md`.
