...
| `py -m unittest discover -s tests -v` | `ForgePact/` | PowerShell / CMD | Python 3.10+ | Executes all 161 Python contract tests. | Read-only test execution; all tests pass | Verified |
...
11. **The Pet Quest Collector ships a hardcoded native address into the player build — and this project has removed a feature for exactly that before:**
    - `PetQuestCollectOne()` reaches the game's call-a-method-value helper as `GetModuleHandleA(nullptr) + kQuestPickupCallFnRva` (`0xB489070`), calls through the resulting pointer, and **validates nothing** — not the module size, not the bytes at the target, not the game build. Every other shipped mechanism in `ModuleMain.cpp` resolves by *name* (`HookOneScript`, `HookBuiltin`, `asset_get_index`), which fails loudly and harmlessly against an unfamiliar build. This one does not: on a game build where that address is something else, the call transfers control into whatever is there.
    - **This is not a hypothetical standard.** `relicgate` was deleted from this plugin for precisely this reason and still answers `bu ozellik kaldirildi (sabit adres oyun guncellemeleriyle kayiyor)` — "removed; the fixed address drifts with game updates". The Pet Quest Collector reintroduces the pattern, in a player-facing mod, in a build that auto-applies its saved settings on every launch.
    - It is shipped anyway because the mechanism was confirmed by reproduction and no name-based route to it is known (the method value is anonymous; `HookOneScript` cannot see it — see the Plan C log's `HookOneScript` correction). Treat it as a **standing item**, not a closed one: the mod should acquire a guard (at minimum, refuse to call when the module's image size or the bytes at the RVA do not match what was measured) before the next Season 10 patch, and **`kQuestPickupCallFnRva` must be re-verified on every game update** — see Maintenance Triggers.
    - Diagnostics if it ever misbehaves: `petquest stat` (research build) reports `collected`, the two skip counters, lost targets and travel timeouts; a crash inside the call would show up in the stall watchdog's sample as an address inside `Hero_Siege.exe`, not inside `BloodPactPlugin.dll`.

---

## Maintenance Triggers

- **Game Executable Updates:** When a new Season 10 patch releases, verify that spawner object names, GML function names, and `LoadDrops` drop family indices remain valid. **Also re-verify `kQuestPickupCallFnRva`** (`ModuleMain.cpp`, the Pet Quest Collector's call-a-method-value helper): unlike everything else here it is a raw address, not a name, so it cannot fail safely — see Known Limitations item 11. `citrace dispatchdump` / `citrace symdump` in the research build are the tools for re-locating it.
